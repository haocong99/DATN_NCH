...
int fire = 0;
const byte relayPin[5] = {2, 25, 17, 16, 4};
...
void sendWarning()
{
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    OCstat_current[i] = ( stat == 1 && digitalRead(relayPin[i]) == 1) ? 1 : 0  ; // Thoa man thi 1 ko thoa man thi 0
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    SCstat_short[i] = (stat_short == 1 && digitalRead(relayPin[i]) == 1) ? 1 : 0;
  }
  for (int x = 0; x < 5; x++)
  {
...
void MQTTPublishRelay()
{
  ...
  for (int i = 0; i < 5; i++)
  {
    int stat = digitalRead(relayPin[i]);
  ...
  for (int x = 0; x < 5; x++)
  {
    ...
    {
      displayRelay();
      int int_RLStat = digitalRead(relayPin[x]);
